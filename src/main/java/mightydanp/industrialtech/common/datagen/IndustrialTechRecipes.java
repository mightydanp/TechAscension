...
import static mightydanp.industrialtech.common.data.IndustrialTechData.ItemFilter;
import static mightydanp.industrialtech.common.data.Materials.*;

public class IndustrialTechRecipes extends AntimatterRecipeProvider {

    public IndustrialTechRecipes(String providerDomain, String providerName, DataGenerator gen) {
    ...
    public void registerRecipes(Consumer<IFinishedRecipe> consumer) {
        ...
        addConditionalRecipe(consumer, getStackRecipe("", "has_sulfur_dust", this.hasItem(getForgeItemTag("dusts/sulfur")),
                new ItemStack(Blocks.TORCH, 6), of('D', getForgeItemTag("dusts/sulfur"), 'R', Tags.Items.RODS_WOODEN), "D", "R"), References.class, "sulfurTorch", References.ID, "sulfur_torch");

        addItemRecipe(consumer, References.ID, "hopper", "", "has_wrench", this.hasItem(WRENCH.getTag()),
        ...
        addItemRecipe(consumer, References.ID, GEAR.get(Wood).getRegistryName().getPath() + "_alt", "gears", "has_wooden_rod", this.hasItem(Tags.Items.RODS_WOODEN),
                GEAR.get(Wood), of('P', ItemTags.PLANKS, 'W', WRENCH.getTag(), 'R', Tags.Items.RODS_WOODEN), "RPR", "PWP", "RPR");
        ...
        addItemRecipe(consumer, References.ID, GEAR.get(Stone).getRegistryName().getPath() + "_alt", "gears", "has_stone", this.hasItem(Tags.Items.STONE),
                GEAR.get(Stone), of('S', Tags.Items.STONE, 'W', WRENCH.getTag(), 'C', Tags.Items.COBBLESTONE), "CSC", "SWS", "CSC");

        addItemRecipe(consumer, "gears", "has_stone", this.hasItem(Blocks.PISTON), Blocks.STICKY_PISTON, of('S', IndustrialTechData.StickyResin, 'P', Blocks.PISTON), "S", "P");
...
//        RegistrationHelper.getMaterialsForDomain(References.ID).stream().filter(m -> m.has(PLATE, INGOT)).forEach(mat -> {
...
//        });
//
//        RegistrationHelper.getMaterialsForDomain(References.ID).stream().filter(m -> m.has(DUST)).forEach(mat -> {
...
//        });
    ...
    private String fixLoc(String attach) {
        return References.ID.concat(":").concat(attach);
    }
