...
public class AssetPackRegistry {
    ...
    public static void init() {
        ...
        blockModelDataMap.forEach((s, b) -> TechAscension.assetHolder.addToResources(new ResourceLocation(Ref.mod_id, "models/" + (b.getParentFolder() == null ? "" : b.getParentFolder() + "/")  + s + ".json"), b.createJson()));
        itemModelDataHashMap.forEach((s, b) -> TechAscension.assetHolder.addToResources(new ResourceLocation(Ref.mod_id, "models/" + (b.getParentFolder() == null ? "" : b.getParentFolder() + "/")  + (s.contains(":") ? s.split(":")[1] : s) + ".json"), b.createJson()));
